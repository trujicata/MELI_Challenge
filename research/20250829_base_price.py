...
# For that we need the "currency_id" column and the "date_created" column
...
# %%
def transform_into_USD(price, currency_id, date_created):
    if currency_id == "USD":
        return price
    else:
        # ensure date_created is string YYYY-MM-DD
...
df["base_price_usd"] = df.apply(
    lambda row: transform_into_USD(
        row["base_price"], row["currency_id"], row["date_created"]
    ),
    axis=1,
)
df["base_price_usd"].describe()
# %%
df["base_price"].describe()
# %%
# do a sns histogram of the base price in USD
bins = [0, 1, 10, 250, 1000, 10000, 20000]
sns.histplot(df["base_price_usd"], bins=bins)
plt.xscale("log")
plt.ylim(0, 50000)
...
sns.histplot(df[df["used"] == 1]["base_price_usd"], bins=bins, ax=axs[0])
axs[0].set_xscale("log")
axs[0].set_ylim(0, 28000)
axs[0].set_title("Base price in USD for used items")

sns.histplot(df[df["used"] == 0]["base_price_usd"], bins=bins, ax=axs[1])
...
# %%
df[df["used"] == 1]["base_price_usd"].describe()
# %%
# do a sns histogram of the base price in USD for new items
df[df["used"] == 0]["base_price_usd"].describe()
# %%
...
# %%
# Look for Pearson correlation between the "used" column and the "base_price_usd" column
pearson_corr = df[["used", "base_price_usd"]].corr(method="pearson")
pearson_corr  # 0.000931
# %%
# Look for Spearman correlation between the "used" column and the "base_price_usd" column
spearman_corr = df[["used", "base_price_usd"]].corr(method="spearman")
spearman_corr  # -0.2025
# %%
# Look for Kendall correlation between the "used" column and the "base_price_usd" column
kendall_corr = df[["used", "base_price_usd"]].corr(method="kendall")
kendall_corr  # -0.1659
